...
-module(ssam_message_sms_resource).
...
post(#request{
	...
	state_post_sms(check_callback_url, Request).



%% Private functions

state_post_sms(check_callback_url, #request{body = Body} = Request) ->
	case proplists:get_value(<<"callback_url">>, Body) of
		?undefined ->
			state_post_sms(check_msg_id, Request);
		CallbackUrl ->
...
state_post_sms(check_msg_id, Request) ->
	...
	case ssam_riakc:get(?BUCKET_MESSAGE_SMS, log_key(Sid, MsgId)) of
		{?error, ?not_found} ->
			state_post_sms(check_receivers, {MsgId, Request});
		{ok, _} ->
...
state_post_sms(check_receivers, {MsgId, Request}) ->
	...
	case length(ToList1) > 0 of
		true ->
			state_post_sms(write_ref, {MsgId, ToList1, Request});
		false ->
			{?error, ssam_rest:response(?STATUS_BAD_REQUEST, ?no_receivers, Request)}
	end;
state_post_sms(write_ref, {MsgId, ToList, Request}) ->
	...
	case ssam_riakc:put(?BUCKET_MESSAGE_SMS, ref_key(RefId), Doc) of
		ok ->
			state_post_sms(write_log, {RefId, MsgId, ToList, Request});
...
state_post_sms(write_log, {RefId, MsgId, ToList, Request}) ->
	...
	Body1 = ssam_objects:list_deleted([<<"id">>, <<"to">>], Body),
	...
	case ssam_riakc:put(?BUCKET_MESSAGE_SMS, log_key(Sid, MsgId), Doc) of
		ok ->
			state_post_sms(dispatch, {RefId, Msg, From, ToList, Doc, Request});
		{?error, Reason} when is_atom(Reason) ->
			{?error, ssam_rest:response(?STATUS_CONFLICT, Reason, Request)};
		{?error, _} ->
			{?error, ssam_rest:response(
...
state_post_sms(dispatch, {RefId, Msg, From, ToList, Doc, Request}) ->
	case ssam_conf:value(?SERVICE, [providers, default, handler]) of
		?undefined ->
			{?error, ssam_rest:response(
					   ?STATUS_SERVICE_UNAVAILABLE, ?provider_handler_not_found, Request)};
		Handler when is_atom(Handler) ->
			case Handler:send_sms(RefId, Msg, From, ToList) of
				ok ->
