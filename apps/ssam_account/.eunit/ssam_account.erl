...
send_mail(To, Sid)->
	TemplatePath = ssam_conf:value(?SERVICE, [mail, welcome, template]),
	TemplatePath1 = erlang:iolist_to_binary([code:priv_dir(ssam), $/, TemplatePath]),
	case file:read_file(TemplatePath1) of
		{ok, Template} -> 
			Profile = ssam_conf:value(?SERVICE, [mail, welcome, profile]),
			From = ssam_conf:value(?SERVICE, [mail, welcome, from]),
			Subject = ssam_conf:value(?SERVICE, [mail, welcome, subject]),
